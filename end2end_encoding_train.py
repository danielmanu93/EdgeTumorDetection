...
def main(args):
    ...
    init_enc = 'gauss'
    if args.up_mode:    
        model = network.model_dict[args.model](upsample_mode=args.up_mode, init = init_enc, train_encoder = args.train_enc).to(device)
    else:
        model = network.model_dict[args.model](init = init_enc, train_encoder = args.train_enc).to(device)
    ...
    lr_scheduler = ReduceLROnPlateau(optimizer, patience = 300, verbose=True)
...
def parse_args():
    ...
    #parser.add_argument('-t', '--train-anno', default='data.txt', help='name of train anno')
    ...
    #parser.add_argument('-v', '--val-anno', default='test.txt', help='name of val anno')
    ...
    parser.add_argument('-te', '--train_enc', type=bool, default=False)
