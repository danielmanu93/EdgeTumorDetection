...
def TV_reg(v,alpha, eps=1e-2):
    gradx = torch.roll(v, 1, dims = -2) -v
    grady = torch.roll(v, 1, dims = -1) -v
    grad_norm_eps = (gradx**2 + grady**2 + eps)**(1/2)
    return alpha*torch.mean(grad_norm_eps)
