...
def log_transform(data, k=1, c=0,eps = 0):
    if eps > 0:
       return (np.log1p(k*((data**2 + eps**2)**(1/2) -eps)  + c)) * np.sign(data)
    else:
       return (np.log1p(np.abs(k * data) + c)) * np.sign(data)

def log_transform_tensor(data, k=1, c=0, eps = 0):
    if eps > 0:
       return (torch.log1p(k*((data**2 + eps**2)**(1/2) -eps)  + c)) * torch.sign(data)
    else:
...
class RootTransform(object):
    def __init__(self, lb,ub, sf):
        self.lb = lb
        self.ub = ub
        self.sf = sf
    def __call__(self,data):
        d_norm = 2*(data-self.lb)/(self.ub-self.lb) -1
        return np.sign(d_norm)*np.abs(d_norm)**(1/self.sf)
class PowerTransform(object):
    def __init__(self, lb,ub, sf):
        ...
        self.sf = sf
    def __call__(self,data):
        d_norm = np.sign(data)*np.abs(d_norm)**self.sf
        d_norm = (self.ub-self.lb)*(d_norm + 1)/2 +self.lb
        return (self.ub-self.lb)*(d_norm + 1)/2 +self.lb
